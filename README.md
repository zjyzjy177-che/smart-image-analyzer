...
| 功能 | 模型/方法 | 输出 |
|------|-----------|------|
| 整图描述 | BLIP Base + MarianMT | 英文描述生成后离线翻译为中文 |
...
- BLIP 逐词生成概率用于计算描述生成可信度，场景大类会标注“可信/仅供参考”。
...
- MarianMT 英译中模型约 298 MB，用于翻译 BLIP 的原始英文描述。
...
- 默认“自动检测（已安装语种）”：优先使用上次成功语言，达到可靠结果后
  立即停止，不再遍历全部模型。
- 未安装相应语种时会提示手动选择；选择后自动下载并缓存该语言模型。
- 在结果图上绘制达到阈值的文字区域框。
- 显示每段文字的置信度。
...
自动模式首次加载模型可能需要十余秒，后续同语种通常在 1 秒左右完成。
...
脚本会优先通过 Windows `py -3` 启动器定位 Python，并检查 Gradio 6、
Transformers 5 等关键依赖，然后打开 `http://localhost:7860`。

**常见问题：**
- 提示“Python 未找到” → 确认已安装 Python Launcher（安装 Python 时默认勾选）
