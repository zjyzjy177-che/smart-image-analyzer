...
> 新环境部署请先阅读 [本地部署使用说明](本地部署使用说明.md)。本项目使用
> Git LFS 保存离线模型，不能用普通 Source code ZIP 代替完整克隆。
...
- ResNet50 权重约 100 MB，随离线项目放在 `models/resnet50/`。
- BLIP Base 权重约 945 MB，随离线项目放在
  `models/blip-image-captioning-base/`。
- 英译中模型约 301 MB，放在 `models/opus-mt-en-zh/`。
...
| OCR 后端 | 语言 | 说明 |
|----------|------|------|
| EasyOCR | 简体中文 + English | 模型随离线项目提供，各环境使用同一识别链路 |

**已完成功能：**

- 支持 RGB、RGBA、灰度及浮点图片。
- 离线版本专注简体中文、英文和数字混排，避免打包大量不使用的语种模型。
- EasyOCR 检测与中英文识别权重放在 `models/easyocr/`。
- 在结果图上绘制达到阈值的文字区域框。
- 显示每段文字的置信度。
...
项目内已集成中英文 EasyOCR 模型，新环境运行时不依赖用户目录中的模型缓存。
...
**Q: 模型下载太慢？**
A: 离线项目包已经在 `models/` 中包含 ResNet50、BLIP、英译中和中英文 OCR
模型，程序会优先读取项目内文件。大型权重通过 Git LFS 管理，克隆仓库时需要
安装 Git LFS 并执行 `git lfs pull`。
...
**Q: 为什么统一使用 EasyOCR？**
A: 中英文离线模型体积更可控，并且可避免不同 Python 环境下 PaddleOCR 推理
后端不一致的问题。
